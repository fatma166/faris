...
class WebController extends Controller
{
    ...
    public function checkout_details(Request $request)
    {
        ...
        if ($zip_restrict_status&&(isset($shipping_type)&&($shipping_type!="city_wise"))) {
            $zip_codes = DeliveryZipCode::where('country_code','SA')->get();//all();
        }elseif(isset($shipping_type)&&($shipping_type=="city_wise")){
            $zip_codes = ShippingMethod::where(['creator_type' => 'admin'])->whereHas('shippingCity')->with('shippingCity')/*->orwhereNULL('city_id')*/->get();
        } else {
            $zip_codes = 0;
        }

        if(isset($shipping_type)&&($shipping_type=="city_wise")){
            $shipping_coast=ShippingMethod::where('city_id',$zip_codes[0]->city_id)->value('cost');

            $shipping_duration=ShippingMethod::where('city_id',$zip_codes[0]->city_id)->value('duration');

             $request->session()->put('shipping_coast',$shipping_coast);
            $shipping_duration=$request->session()->put('shipping_duration',$shipping_duration);
        }elseif(isset($shipping_type)&&($shipping_type=="order_wise")){
            $shipping_coast=ShippingMethod::where(['status'=>1])->whereNULL('city_id')->value('cost');

            $shipping_duration=ShippingMethod::where(['status'=>1])->whereNULL('city_id')->value('duration');

            $request->session()->put('shipping_coast',$shipping_coast);
            $shipping_duration=$request->session()->put('shipping_duration',$shipping_duration);
        }elseif(isset($shipping_type)&&($shipping_type=="category_wise")){
             $shipping_coast=CategoryShippingCost::value('cost');

            $shipping_duration=CategoryShippingCost::value('duration');

            $request->session()->put('shipping_coast',$shipping_coast);
            $request->session()->put('shipping_type',$shipping_type);
            $shipping_duration=$request->session()->put('shipping_duration',$shipping_duration);
        ...
        $shipping_coast=$request->session()->get('shipping_coast');
       // $shipping_duration=$request->session()->get('shipping_duration');


        if (count($cart_group_ids) > 0) {
            return view('web-views.checkout-shipping', compact('physical_product_view', 'zip_codes', 'country_restrict_status', 'zip_restrict_status', 'countries','shipping_type','shipping_coast'));
    ...
    public function checkout_payment(Request $request)
    {
        ...
        $shipping_arr=\App\Model\ShippingType::where('seller_id', 0)->first();
        $shipping_type=$shipping_arr->shipping_type ;
        if(isset($shipping_type)&&($shipping_type=="city_wise")){
            $shipping_coast=$request->session()->get('shipping_coast');
            $shipping_duration=$request->session()->get('shipping_duration');
        }elseif(isset($shipping_type)&&($shipping_type=="order_wise")) {
        ...
        if(isset($request->date)){
            if(($request->date)<(Carbon::now()->addDays($shipping_duration))){
                return response()->json(['error' => " min shipping duration is ".$shipping_duration."day",'type'=>'date_error']);
            }
            // exit;
        }
        ...
        if (session()->has('address_id') && count($cart_group_ids) > 0) {
            return view('web-views.checkout-payment', compact('cod_not_show','shipping_coast','shipping_duration'));
        }
    ...
    public function digital_product_download($id)
    {
        ...
        if( $order_data->product->digital_product_type == 'ready_product' && $order_data->product->digital_file_ready) {
            $file_path = storage_path('app/product/digital-product/' .$order_data->product->digital_file_ready);
        }else{
            $file_path = storage_path('app/product/digital-product/' . $order_data->digital_file_after_sell);
        }
    ...
    public function get_country_cities(Request $request){
        $cities=DeliveryZipCode::where('country_code',$request->country_code)->get()->toAarray();
        return($cities);
    }
    public function change_get_shipping_cost(Request $request){
        $city_id=$request->city_id;
       // $shipping_type="city_wise";
        $shipping_type=$request->session()->get('shipping_coast');
        $ajax="ajax";
        if($shipping_type=="city_wise") {
            $request->session()->forget('shipping_coast');
            $shipping_coast = ShippingMethod::where('city_id', $city_id)->value('cost');
            $shipping_duration = ShippingMethod::where('city_id', $city_id)->value('duration');
            $request->session()->put('shipping_coast', $shipping_coast);
            $request->session()->put('shipping_duration', $shipping_duration);
            $shipping_coast = $request->session()->get('shipping_coast');
            return view('web-views.partials._order-summary',compact('city_id','shipping_type','ajax','shipping_coast'));
        }else{
            return response()->json(['shiping' =>$shipping_type."day",'type'=>'date_error']);
        }
      //  $cost=ShippingMethod::where('city_id',$request->city_id)->value('cost');
    ...
    public function add_order_date(Request $request){
        ...
        if($date==""){
            return response()->json(['errors' => 'set shipping day first']);
            }
        $shipping_duration=$request->session()->get('shipping_duration');
        ...
        if(isset($request->date)){
            if(($date)<(Carbon::now()->addDays($shipping_duration))){

                return response()->json(['errors' => " min shipping duration is ".$shipping_duration."day",'type'=>'date_error']);
            }
            // exit;
        }
