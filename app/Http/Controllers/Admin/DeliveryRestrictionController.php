...
class DeliveryRestrictionController extends Controller
{
    ...
    public function addZipCode(Request $request)
    {
        ...
        $city_codes = explode(',' ,$request->city);
        $existing_city = DeliveryZipCode::pluck('city')->toArray();

        $city_codes = array_diff($city_codes, $existing_city);
        if (!$city_codes) {
            Toastr::warning(translate('Delivery_city_already_exists!'));
            return back();
        }

        if(count($city_codes)!= count($zip_codes)){
            Toastr::warning(translate('Delivery_city_and_zipcodes_mustbe_same_length!'));
            return back();
        }

        $data = array();
        foreach ($zip_codes as $index=>$code)
        {

            $data[] = array(
                'zipcode' => $code,
                'country_code'=>$request->country_code,
                'city'=>$city_codes[$index],
