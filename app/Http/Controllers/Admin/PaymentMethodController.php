...
class PaymentMethodController extends Controller
{
    ...
    public function update(Request $request, $name)
    {
        ...
        if ($name == 'digital_payment') {
            ...
            $payment = BusinessSetting::where('type', 'tamara')->first();
            if (isset($payment) == false) {
                DB::table('business_settings')->insert([
                    'type' => 'tamara',
                    'value' => json_encode([
                ...
                DB::table('business_settings')->where(['type' => 'tamara'])->update([
                    ...
                    'value' => json_encode([
                        ...
                        'tamara_notification_token' => $request['tamara_notification_token'],
                    ]),
                    'updated_at' => now()
                ]);
            }
        } elseif ($name == 'tabby') {
            $payment = BusinessSetting::where('type', 'tabby')->first();
            if (isset($payment) == false) {
                DB::table('business_settings')->insert([
                    'type' => 'tabby',
                    'value' => json_encode([
                ...
                if ($request['status'] == 1) {
                    $request->validate([
                        'tabby_api_key' => 'required',
                        //'tabby_secret' => 'required'
                    ]);
                }
                DB::table('business_settings')->where(['type' => 'tabby'])->update([
                    'type' => 'tabby',
                    'value' => json_encode([
                        'status' => $request['status'],
                        'environment' => $request['environment'],
                        'tabby_api_key' => $request['tabby_api_key'],
                        //'tabby_secret' => $request['tabby_secret'],
                    ]),
                    'updated_at' => now()
                ]);
            }
