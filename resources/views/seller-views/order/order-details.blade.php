                        ...
                        <div class="media">
                            <div class="mr-3">
                                <img class="avatar rounded avatar-70" onerror="this.src='https://6valley.6amtech.com/assets/front-end/img/image-place-holder.png'"
                                     src="{{!empty($order->seller->shop) ? asset('storage/app/seller/'.auth('seller')->user()->image) : ''}}"
    ...
    <script>
        ...
        function initializegLocationMap() {
            ...
            google.maps.event.addListener(marker, 'click', (function (marker) {
                return function () {
                    infowindow.setContent("<div style='float:left'><img style='max-height:40px;wide:auto;' src='{{asset('storage/app/profile/')}}{{$order->customer->image??""}}'></div><div style='float:right; padding: 10px;'><b>{{$order->customer->f_name??""}} {{$order->customer->l_name??""}}</b><br/>{{$shipping_address->address}}</div>");
