                            ...
                            <div class="card-body p-0">
                                <ul class="list-group">
                                    @foreach($shipping_addresses as $key=>$address)
                                        <li class="list-group-item __inline-57" onclick="set_shipping('{{$address['city']}}')" onclick="$('#sh-{{$address['id']}}').prop( 'checked', true )">
                                            ...
                                            <div class="d-flex">
                                                ...
                                                <div class="w-0 flex-grow-1 justify-content-between">
                                                    ...
                                                    <span>{{ \App\CPU\translate('address')}} : {{$address['address']}}, </span><span class="checked_city">{{$address['city']}}</span>, <span class="checked_zip">{{$address['zip']}}</span>
                                                </div>
                                                <div class="">
                                                            ...
                                                            <div class="cities">
                                                                <select name="zip" class="form-control selectpicker" data-live-search="true" required>
                                                                    @forelse($zip_codes as $code)
                                                                        <option value="{{ $code->zipcode}}">{{ $code->city}}</option>
                                                                    @empty
...
{{--                                        {{$billing_addresses->count()==0?'checked':''}}--}}
                                        ...
                                        <div id="accordion">
                                            <div id="billing_model"
                                                ...
                                                <div class="card-body">
                                                    ...
                                                    <div class="form-group bil_city_div">
                                                        <label
                                                            for="exampleInputEmail1">{{ \App\CPU\translate('zip_code')}}
                                                            <span class="text-danger">*</span></label>
                                                        @if($zip_restrict_status)
                                                            ...
                                                            <input class="hidden billing_city"  name="billing_city" @if($zip_codes[0]->city) value="{{$zip_codes[0]->city}}" @endif/>
                                                        @else
                                                            <input type="text" class="form-control"
                        ...
                        <h2 class="h4 pb-3 mb-2 mt-5">{{ \App\CPU\translate('choose_order_date')}}</h2>
                        @php($shipping_addresses=\App\Model\ShippingAddress::where('customer_id',auth('customer')->id())->where('is_billing',0)->get())
                        <form method="post" class="card __card" id="order-date-form">
                            <div class="card-body p-0">
                                <div class="form-floating">
                                    <input type="date" name="order_date"  id="order_date"
                                           class="form-control">
                                  <!--  <label>{{--\App\CPU\translate('Order_Delivery_Date')--}}</label>-->
                                </div>
                            </div>
                        </form>
...
@push('script')
    ...
    <script>
        ...
        $("select[name='country']").on('change',function(){
            $.ajax({
                url:"<?php route('get-country-cities') ?>",
                type:'get',
                data:{country_code:this.value},
                success:function (data) {
                     $(".cities select").empty();
                    /* $.each(data,function(index,city_data))
                        {
                            var $newoption=$("<option></option>").val(city_data.zipcode).text(city_data.zipcode);
                            $(".cities select").append($newoption).trigger('change');
                        }*/
        ...
        $(".cities select").on('change',function() {
            ...
            $.ajax({
                url:"{{route('change-get-shipping-cost')}}",
                type:'get',
    ...
    <script>
       ...
       // var timer='';
        ...
        $("select[name='billing_zip']").on('change',function() {
            var optionSelected = $(this).find("option:selected");
            // var valueSelected  = optionSelected.val();
            var textSelected   = optionSelected.text();
            $(".billing_city").val(textSelected);
